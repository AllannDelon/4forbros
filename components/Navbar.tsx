...
export default function Navbar() {
  ...
  // For anchor links: if already on home, scroll smoothly; otherwise navigate
  const handleAnchorClick = (e: React.MouseEvent, href: string) => {
    if (!href.startsWith("/#")) return; // let normal Link handle /veiculos etc.
    e.preventDefault();
    const anchor = href.slice(1); // e.g. "#inicio"
    if (pathname === "/") {
          ...
          {NAV_LINKS.map(({ label, href }) => (
            <Link
              ...
              className={`text-sm font-inter font-medium transition-colors ${
                (href === "/veiculos" && pathname === "/veiculos")
                  ? "text-white"
                  : "text-[#8b90a1] hover:text-white"
              }`}
            >
              {label}
            </Link>
